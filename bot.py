...
try:
    ...
    logger.error("Generate new one using: python -c \"from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())\"")
...
async def create_wallet_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cipher:
        try:
            ...
            await update.message.reply_text("❌ Encryption error. Try again.")
            return
    else:
        encrypted_key = wallet['private_key']
    
    success = db.create_user(user_id, username, wallet['address'], encrypted_key)
    ...
    if success:
        ...
        await update.message.reply_text("❌ Error saving wallet. Try again.")
...
if __name__ == "__main__":
    try:
        ...
        if "event loop" in str(e):
            logger.info("Bot stopped due to event loop error")
        else:
